  ...
  if (!(BackList = SUMA_OverlaysToOrderedList (ado, -1))) {
   ...
   switch (ado->do_type) {
      case SO_type: {
        ...
        fprintf(stderr, "SUMA_AddNewPlane: OK 9e\n");
            kin = 2;
         } else kin = 1;
         if (dov) {
...
SUMA_Boolean SUMA_MixColors (SUMA_SurfaceViewer *sv)
{
   ...
   for (i=0; i<sv->N_ColList; ++i){
    fprintf(stderr, "Processing color list %d of %d\n", i, sv->N_ColList);
      ...
      {/* Check on extras */
         switch (tp) {
            ...
            fprintf(stderr, "Finished processing color lists\n");
