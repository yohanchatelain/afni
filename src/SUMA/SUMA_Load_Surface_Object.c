...
{  /* start SUMA_Load_Spec_Surf */
   ...
   fprintf(stdout, "1: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "2: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "3: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "4: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "5: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "6: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "7: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "8: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "9: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "10: Surface type: %s\n", Spec->SurfaceType[i]);
   ...
   fprintf(stdout, "11: Surface type: %s\n", Spec->SurfaceType[i]);
...
SUMA_SurfaceObject * SUMA_Load_Spec_Surf_with_Metrics(
                           ...
                           int debug)
{
    fprintf(stderr, "SUMA_Load_Spec_Surf_with_Metrics\n");
...
{/* SUMA_LoadSpec_eng */
    fprintf(stderr, "SUMA_LoadSpec_eng\n");
   ...
   fprintf(stderr, "Num. mappable surfaces: %d\n", Spec->N_Surfs);
   for (i=0; i<Spec->N_Surfs; ++i) { /* first loop across mappable surfaces */
      ...
      if (SUMA_iswordin(Spec->LocalDomainParent[i],"SAME") == 1) {
         /* Mappable surfaces */
         fprintf(stderr, "Mappable surfaces\n");
         ...
         fprintf(stderr, "tmpVolParName: %s\n", tmpVolParName);
         ...
         fprintf(stderr, "Check the filename match, to get around ID collision\n");
         ...
         SurfIn = NOPE;
         fprintf(stderr, "Spec->LabelDset[i][0] = %d\n", Spec->LabelDset[i][0]);
         if (SO && Spec->LabelDset[i][0] != '\0') {
            ...
            fprintf(stderr, "do some deeds on this overlay\n");
         ...
         if (SUMA_isEnv("SUMA_AutoLoad_Matching_Dset","YES")){
            fprintf(stderr, "dsets identically named\n");
             SUMA_AutoLoad_SO_Dsets(SO);
         }
      }/* Mappable surfaces */
   }/* first loop across mappable surfaces */

   fprintf(stderr, "Num. non-mappable surfaces: %d\n", Spec->N_Surfs);
   for (i=0; i<Spec->N_Surfs; ++i) { /* Now locate and load all
      ...
      fprintf(stderr, "tmpVolParName: %s\n", tmpVolParName);
      fprintf(stderr, "Spec->LocalDomainParent: %s\n", Spec->LocalDomainParent[i]);
      ...
      if (SUMA_iswordin(Spec->LocalDomainParent[i],"SAME") != 1) {
        ...
        fprintf(stderr, "check if surface read was unique\n");
         /* check if surface read was unique
            it's inefficient to check after the surface is read,
            but idcode is generated in the read routine
            and users should not be making this mistake too often */
...
{
    fprintf(stderr, "SUMA_IO_args_2_spec\n");
   ...
   *nspec = 1;
   ...
   if (ps->accept_i) {
      ...
      for (i=0; i<ps->i_N_surfnames; ++i) {
         ...
         fprintf(stdout, "2: SUMA_SurfaceTypeString: %s\n", SUMA_SurfaceTypeString (ps->i_FT[i]));
   ...
   if (ps->accept_ipar) {
      ...
      for (i=0; i<ps->ipar_N_surfnames; ++i) {
         ...
         fprintf(stdout, "3: SUMA_SurfaceTypeString: %s\n", SUMA_SurfaceTypeString (ps->i_FT[i]));
         strcpy(  spec->SurfaceType[spec->N_Surfs],
   ...
   if (ps->accept_t) {
      ...
      for (i=0; i<ps->t_N_surfnames; ++i) {
         ...
         fprintf(stdout, "4: SUMA_SurfaceTypeString: %s\n", SUMA_SurfaceTypeString (ps->i_FT[i]));
         strcpy(  spec->SurfaceType[spec->N_Surfs],
